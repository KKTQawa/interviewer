        ...
        <div class="loginform-box">
            ...
            <Transition name="form-switch" mode="out-in">
                ...
                <form autocomplete="off" v-if="!isRegisterMode" class="login-form">
                    <div class="input-box">
                        <n-input v-model:value="logininput_info.username" type="text" placeholder="请输入用户名"
                            :maxlength="30" round>
                            <template #prefix>
                                <n-icon>
                                    <UserOutlined />
                    ...
                    <div class="input-box">
                        <n-input v-model:value="logininput_info.password" type="password" placeholder="请输入密码"
                            :maxlength="12" round show-password-on="click">
                            <template #prefix>
                                <n-icon>
                                    <LockOutlined />
                                </n-icon>
                ...
                <form autocomplete="off" v-else @submit.prevent="handleRegister" class="register-form">
                    <div class="input-box">
                        <n-input v-model:value="registerinput_info.username" type="text" placeholder="请输入4-30位用户名"
                            :maxlength="30" round>
                            <template #prefix>
                                <n-icon>
                                    <UserOutlined />
                    ...
                    <div class="input-box">
                        <n-input v-model:value="registerinput_info.email" type="email" placeholder="请输入邮箱" round>
                            <template #prefix>
                                <n-icon>
                                    <MailOutlined />
                                </n-icon>
                    ...
                    <div class="input-box">
                        <n-input v-model:value="registerinput_info.password" type="password" placeholder="请输入8-12位密码"
                            :maxlength="12" round show-password-on="click">
                            <template #prefix>
                                <n-icon>
                                    <LockOutlined />
                    ...
                    <div class="input-box">
                        <n-input v-model:value="registerinput_info.confirmPassword" type="password"
                            placeholder="请再次输入密码" :maxlength="12" round show-password-on="click">
                            <template #prefix>
                                <n-icon>
                                    <LockOutlined />
                                </n-icon>
...
async function performLogin(input_info) {
    ...
    try {
        // 1. 验证用户是否存在
        const response = await axios.get('/user/get_info', {
            params: {
...
async function handleLoginSuccess(input_info, res) {
    try {
        const tokenRes1 = await axios.post('/auth/login', input_info);
        let tokenRes = tokenRes1.data;
        ...
        store.userInfo = {
            ...
            email: res.email,       // 用户邮箱
            avatarSrc: res.preference?.avatarSrc || '', // 用户头像的URL
            bio: res.preference?.bio || '还没有添加任何自我介绍---', // 用户简介
        }
        store.login(); // 设置登录状态
