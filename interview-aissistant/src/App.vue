<template>
  ...
  <div id="modal-root" style="z-index:2000;"></div>
  <div id="modal-ResumeEdit" style="z-index:5000;"></div>
</template>
<script setup>
// ---------------------------------------------------------------*/
//左侧主菜单
...
const items = reactive([
  getMenuItem('主页', '/', () => h(HomeOutlined)),
  getMenuItem('我的', '/myProfile', () => h(UserOutlined)),
  getMenuItem('关于', '/about', () => h(InfoCircleOutlined)),
]);
...
const collapsed = ref(true); // 修改初始状态为展开
...
.main-content {
  ...
  display: flex; /* 只需要写一次 */
  background-color: rgba(78, 61, 61, 0.511);
}
.left-main-menu {
  position: relative;
  height: 100%;
