...
const handleChange = (e) => {
  ...
  reader.onload = async (event) => {
    ...
    try {
      ...
      const res1 = await axios.post('/user/update', {
        ...
        preferences: {
          avatar: res.data.url,
          bio: store.userInfo.bio
        }
      })
      console.log('更新成功', res1.data)
      store.userInfo.avatarSrc = res.data.url
      nextTick();
...
onMounted(() => {
  if (store.userInfo.avatarSrc)
    imageSrc.value = store.userInfo.avatarSrc
})
// 组件卸载时清理
onUnmounted(() => {
  if (CROPPER) {
